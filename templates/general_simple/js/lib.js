 ...
 *  VERSION: 4.9.3
 *  LICENSE: FL04I5GPB9DC - https://www.flynax.com/flynax-software-eula.html
 *  PRODUCT: General Classifieds
 *  DOMAIN: gmowintour.com
 *  FILE: INDEX.PHP
 *  
    ...
    enquire.register("screen and (min-width: 1200px)", {
		...
		match: function(){
            ...
            if (rlPageInfo['controller'] == 'listing_details'){
                $('#area_listing').append($('.area-listing-stats div.stat-data'));
                $('div.stat-data').removeClass('d-none')
            }
		},
		unmatch: function(){
			if (rlPageInfo['controller'] == 'listing_details'){
                $('.main-section').after($('div.listing-details div.stat-data'));
                $('div.stat-data').addClass('d-none')
            }
		}
	});
...
var flynaxTplClass = function(){
	...
	**/
	this.menu = function(){
		...
		menu.removeClass('more_active');
		menu.find('li:not(.more)').attr('style', '');
		$('ul#main_menu_more').hide();

		/* build menu */
		var width = menu.width(),
			buttonWidth = menu.find('li.more').width() + 32,
	...
	**/
	this.mobileMenu = function(){
		...
		$('section.main-menu span.menu-button').unbind('click').bind('click', function(){
			$('section.main-menu ul.menu').toggleClass('show');
		});
	...
	**/
    ...
    this.categoryAlphabet = function(){
            ...
            if ( $('div#cat_alphabet_cont #cont_'+char).length > 0 ) {
                ...
                $.getJSON(rlConfig['ajax_url'], {mode: 'category', item: char, lang: rlLang}, function(response) {
                    if ( response != '' && response != null ) {
                        cont = '<ul class="char_cont row" id="cont_'+char+'">';
                        for (var i = 0; i < response.length; i++ ) {
                            ...
                            cont += '<li class="col-xl-2 col-lg-3 col-md-4 col-sm-6"><a title="'+response[i].name+' ('+response[i].Count+')" href="'+url+'">'+response[i].name+'</a></li>';
                        }
                        cont += '</ul>';
                    }
                    else {
                        console.log(lang);
                        cont = '<div class="char_cont char_cont_empty" id="cont_'+char+'">'+lang['nothing_found_for_char'].replace('{char}', replace)+'</div>';
                    }
        ...
        $('#cat_alphabet_cont div.close').bind('click touchstart', function(){
            $('div#cat_alphabet_cont').hide();
            $('div.category-alphabet li.active').removeClass('active');
        });

        $(document).bind('click touchstart', function(event){
            if ( !$(event.target).parents().hasClass('category-alphabet-container') ) {
                $('div#cat_alphabet_cont').hide();
                $('div.category-alphabet li.active').removeClass('active');
            }
        });
...
var printMessage = function(type, message, fields, direct){
	...
	this.build = function(){
		...
		$('.notification').css('top', '-'+height+'px').show().animate({top: offset}, 'slow');
	...
	this.closeMessage = function(build){
		...
		$('.notification').animate({top: 50, opacity: 0}, function(){
			$('.notification').remove();
			if ( build ) {
				self.build();
			}
	...
	var notificationScroll = function(){
		if ( this.isFixed() ) {
			...
			$('.notification').removeClass('hide fixed').removeAttr('style');
		}
	};
