    ...
    **/
    public function hookContactSellerFormAboveMessage()
    {
        ...
        if (($page_info['Controller'] == 'listing_details' && in_array($this->vacancy_category_id, $category_ids))
            || ($page_info['Controller'] == 'account_type' && $account['Type'] == $this->employer_type_key)
        ) {
...
VS;
    ...
    **/
    ...
    {
        if ($GLOBALS['blocks']['search_on_map']) {
            $GLOBALS['rlStatic']->addBoxHeaderCSS(RL_TPL_BASE . 'components/search-on-map-box/search-on-map-box.css', 'search_on_map');
        }
    }
